...
#if 0
static void router_worker_work_loop(gearman_worker_st *worker)
{
    gearman_return_t rc;
...
static int worker_lua_initialize(lua_State *L)
{
    ...
    worker_ctx_t **w = (worker_ctx_t **)lua_newuserdata(L, sizeof(worker_ctx_t*));
    if (NULL == w) {
    ...
    router_worker = (worker_ctx_t *)malloc(sizeof(worker_ctx_t));
    if (NULL == router_worker) {
        fprintf(stderr, "can not alloc worker ctx\n");
        lua_pushboolean(L, FALSE);
        return 1;
    }
...
static int worker_lua_work_loop(lua_State *L)
{
    ...
    /* while(1) { */
        rc = gearman_worker_work(&w->worker);
        if (gearman_failed(rc)) {
            fprintf(stderr, "%s\n", gearman_strerror(rc));
            fprintf(stderr, "Gearman router worker work error.\n");
            /* assert(0); */
